...
final class Trigger
{
    ...
    private int $lastKeepalivePingAt = 0;

    private array $events = [];

    private int $bootTime;

    private string $replicationCacheKey;

    private string $resetCacheKey;

    private string $restartCacheKey;

    private array $defaultSubscribers = [
        Subscribers\Trigger::class,
        Subscribers\Terminate::class,
        Subscribers\Heartbeat::class,
    ];

    public function __construct(private string $name = 'default', private array $config = [])
    {
        $this->bootTime = time();
     ...
     * Builder config.
     */
    public function configure(bool $keepUp = true): Config
    {
    ...
    public function on(string $table, array|string $eventType, null|array|callable|Closure|string $action = null): void
    {
        ...
        if (str_contains($table, ',')) {
            collect(explode(',', $table))->transform(fn ($table) => mb_trim($table))
                ->filter()
        ...
        if (is_string($eventType)) {
            ...
            if (str_contains($eventType, ',')) {
                collect(explode(',', $eventType))
                    ->transform(fn ($eventType) => mb_trim($eventType))
                    ->filter()
