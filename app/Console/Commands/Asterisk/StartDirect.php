...
#[Signature('asterisk:start-direct')]
#[Description('Start the Asterisk ARI Listener Manager')]
final class StartDirect extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Periodic Server Check (Every 5 seconds)
        $loop->addPeriodicTimer(5.0, function () use ($loop) {
            $this->initializeServer($loop);
        });
    ...
    private function initializeServer(LoopInterface $loop): void
    {
    ...
    private function connectToServer(Server $server, LoopInterface $loop): void
    {
        ...
        $appName = 'MyStasisApp';
        $base = $server->ari_host.($server->ari_host ? ":$server->ari_port" : '');

        $url = "ws://$base/ari/events?api_key=$server->ari_username:$server->ari_password&app=$appName";

        connect($url, [], [], $loop)->then(function ($conn) use ($server) {
            $this->components->task("Connected to $server->ari_host", fn () => true);
            ...
            $conn->on('close', function ($code = null) use ($server) {
                $this->components->warn("Connection closed: $server->ari_host (code: $code)");
